...
import { range as rangeArr, getFirstDayOfMonth, getPrevMonthLastDays, getMonthDays, validateRangeInOneMonth } from 'element-ui/src/utils/date-util'
export default {
  inject: ['elCalendar'],
  props: {
  ...
  data() {
    return {
      WEEK_DAYS: ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'].map(week => this.$t(`el.datepicker.weeks.${week}`))
    }
  },
  computed: {
    prevMonthDatePrefix() {
